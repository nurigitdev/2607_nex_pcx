...
database, and remote providers together. App-host process templates are documented in
`docs/service_process_startup.md` and generated by
`scripts/render_service_startup_templates.py`. Emergency recovery commands are
indexed in `docs/emergency_recovery_commands.md`. Operator handoff bundle export
is documented in `docs/operator_handoff_bundle.md`. Release tag snapshots are
documented in `docs/release_tag_version_snapshot.md`.

Generate app-host env/systemd startup templates:
...
Render the emergency recovery command index:
...
Export an operator handoff bundle:
...
Export a release version snapshot and tag command guide:
...
Run the DGX Spark remote embedding worker E2E smoke to create a temporary chunk,
process one embedding job per profile through route-aware workers, verify vector
persistence, and clean up the fixture:
