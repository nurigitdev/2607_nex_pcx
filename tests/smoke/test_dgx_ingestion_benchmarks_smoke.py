...
def test_dgx_ingestion_benchmark_compare_page_without_database(client) -> None:
    response = client.get("/admin/dgx-ingestion-benchmarks/compare")

    assert response.status_code == status.HTTP_200_OK
    assert "DGX Benchmark Run 비교" in response.text
    assert "data-dgx-benchmark-compare-page" in response.text
    assert "/api/admin/dgx-ingestion-benchmarks/compare" in response.text
    assert "NEX_PCX_DATABASE_URL is not configured." in response.text
