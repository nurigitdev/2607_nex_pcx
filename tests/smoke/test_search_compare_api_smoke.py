...
def test_search_runtime_failures_api_requires_database_url() -> None:
    app = create_app(Settings(database_url=None))

    with TestClient(app) as client:
        response = client.get("/api/search/logs/runtime-failures")
...
def test_search_latency_outliers_api_requires_database_url() -> None:
    app = create_app(Settings(database_url=None))

    with TestClient(app) as client:
        response = client.get("/api/search/logs/latency-outliers")

    assert response.status_code == 503
    assert response.json() == {"detail": "NEX_PCX_DATABASE_URL is not configured."}


def test_search_no_result_logs_api_requires_database_url() -> None:
    app = create_app(Settings(database_url=None))

    with TestClient(app) as client:
        response = client.get("/api/search/logs/no-results")
...
def test_search_operations_summary_api_requires_database_url() -> None:
    app = create_app(Settings(database_url=None))

    with TestClient(app) as client:
        response = client.get("/api/search/logs/operations-summary")
