...
| `NP-SRC-02` | Canonical terminology, states, API contract | Shared terms, states, error response, pagination, idempotency, and schema versioning should be normalized early. | PCX accumulated many API/UI slices where stable naming reduced rework. | Shared | MVP | Common module, API | Which contract fields must be frozen before service implementation? | Promote to common contract review lane. | Open |
| `NP-SRC-03` | Common foundation | Foundation packages should define logging, settings, job runtime, trace, database, and design system boundaries. | PCX quality gate, logging, job queue, and runtime scripts became cross-cutting patterns. | Shared | MVP | Common module, testing | What belongs in a shared package vs service-local code? | Review after service boundary confirmation. | Open |
